...
program.command('init')
  .description('Initialize a new Nuxt.js project with Hasura and GraphQL setup.')
  .argument('<project-name>', 'Name of the Nuxt.js project')
  .action(async (projectName: string) => {
    console.log(chalk.green(`Initializing new Nuxt.js project: ${projectName}...`));
    try {
      await executeCommand(`npx nuxi init ${projectName}`);
      console.log(chalk.green(`Nuxt.js project '${projectName}' created successfully!`));
    } catch (error) {
      console.error(chalk.red(`Failed to create Nuxt.js project: ${(error as Error).message}`));
    }
  });

program.parse(process.argv);

program.command('hasura')
...
program.command('codegen')
  ...
  .action(async () => {
    ...
    const answers = await inquirer.prompt([
      {
        type: 'input',
        name: 'schemaPath',
        message: 'Enter the path to your GraphQL schema file (e.g., ./schema.graphql):',
        default: './schema.graphql',
      },
      {
        type: 'input',
        name: 'outputDir',
        message: 'Enter the output directory for generated types (e.g., ./types/graphql):',
    ...
    try {
      ...
      await executeCommand('pnpm add -D @graphql-codegen/cli @graphql-codegen/typescript @graphql-codegen/typescript-operations @graphql-codegen/typescript-vue-apollo');
...
module.exports = {
  ...
  generates: {
    "${outputDir}/": {
      ...
      console.log(chalk.green('GraphQL Codegen configuration created in codegen.ts.'));
      ...
      console.error(chalk.red(`Failed to set up GraphQL Codegen: ${(error as Error).message}`));
    }
  });

program.command('get-schema')
  ...
  .action(async () => {
    ...
    const answers = await inquirer.prompt([
      ...
      },
      {
        type: 'input',
        name: 'outputPath',
        message: 'Enter the output path for the schema file (e.g., ./schema.graphql):',
        default: './schema.graphql',
      },
    ]);
    ...
    try {
      ...
      console.log(chalk.green('get-graphql-schema installed.'));

      console.log(chalk.blue(`Fetching schema from ${schemaEndpoint} to ${outputPath}...`));
      await executeCommand(`get-graphql-schema ${schemaEndpoint} > ${outputPath}`);
      console.log(chalk.green('GraphQL schema fetched successfully!'));

    } catch (error) {
      console.error(chalk.red(`Failed to fetch GraphQL schema: ${(error as Error).message}`));
    }
  });

program.command('graphql-client')
  ...
  .action(async () => {
    ...
    try {
      if (client === 'Apollo') {
        ...
        console.log(chalk.yellow('Please remember to add "@nuxtjs/apollo" to your nuxt.config.ts modules.'));
      } else if (client === 'Urql') {
        ...
        console.log(chalk.yellow('Please remember to configure Urql in your Nuxt.js project.'));
      }
      console.log(chalk.green(`${client} GraphQL client set up successfully!`));
    } catch (error) {
      console.error(chalk.red(`Failed to set up GraphQL client: ${(error as Error).message}`));
    }
  });
...
program.command('config')
  ...
  .action(async () => {
    ...
    try {
      ...
      console.log(chalk.green('Configuration saved to .create-nuxt-hasura-cli.json'));
    } catch (error) {
      console.error(chalk.red(`Failed to save configuration: ${(error as Error).message}`));
    }
  });
